    ...
    fun main(args: Array<String>) {
        ...
        for (name in modelNames) {
            try {
                ...
                // Additional debug: attempt to parse resourceBytes as a DJL NDList using NDManager.
                try {
                    val manager = ai.djl.ndarray.NDManager.newBaseManager()
                    java.io.ByteArrayInputStream(resourceBytes).use { ins ->
                        val loaded = try {
                            ai.djl.ndarray.NDList.load(ins, manager)
                        } catch (e: Throwable) {
                            null
                        }
                        if (loaded != null) {
                            println("[Export] NDList parsed from params: size=${loaded.size}")
                            for ((i, nd) in loaded.withIndex()) {
                                println("[Export] ND[$i] shape=${nd.shape} dtype=${nd.dataType}")
                            }
                        } else {
                            println("[Export] NDList.parse: resource did not parse as NDList (or unsupported format)")
                        }
                    }
                } catch (e: Throwable) {
                    System.err.println("[Export] NDList parse attempt threw: ${e.message}")
                    e.printStackTrace()
                }
